...
import com.jfposton.ytdlp.utils.StringUtil;
...
public class YtDlp {
    ...
    public static YtDlpResponse execute(YtDlpRequest request, DownloadProgressCallback callback) throws YtDlpException {
        ...
        List<String> split = StringUtil.splitCommandLine(command);
        ...
        ytDlpResponse = new YtDlpResponse(command, request.getOptions(), directory, exitCode, elapsedTime, out, err);
    ...
    public static VideoInfo getVideoInfo(String url) throws YtDlpException {
        ...
        try {
            // Parse result
            videoInfo = new ObjectMapper().readValue(response.getOut(), VideoInfo.class);
        } catch (IOException e) {
